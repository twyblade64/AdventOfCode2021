...
int main() {
	std::ifstream data("input_19_2.txt");
	...
	while (std::getline(data, line)) {
		...
		if (line[1] == '-') {
			...
			scannerPos.push_back(Vector3(0));
		}
	}
				...
				for (int i = 0; i < scanner.size() && !match; ++i) {
					for (int j = 0; j < mappedProbes.size() && !match; ++j) {
						...
						if (matches.size() >= 12) {
							...
							scannerPos[s] = offset;;
							std::cout << "Match " << s << " " << offset.x << "," << offset.y << "," << offset.z
									  << std::endl;
						}
					}
				}
	...
	for (int i = 0; i < scannerPos.size() - 1; ++i)
		for (int j = i + 1; j < scannerPos.size(); ++j)
			maxDist = max(abs(scannerPos[i].x - scannerPos[j].x) + abs(scannerPos[i].y - scannerPos[j].y) +
						  abs(scannerPos[i].z - scannerPos[j].z), maxDist);
